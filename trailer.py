...
video.with_audio("./resources/bbb.mp4")
intro_clip: VideoClip = video.subclipped(1, 11)
bird_clip: VideoClip = video.subclipped(16, 20)
bunny_clip: VideoClip = video.subclipped(37, 55)
rodents_clip: VideoClip = video.subclipped(
    "00:03:34.75", "00:03:56"
)  # we can also use string notation with format HH:MM:SS.uS
rambo_clip: VideoClip = video.subclipped("04:41.5", "04:44.70")
...
rodents_clip: VideoClip = rodents_clip.with_section_cut_out(start_time=4, end_time=10)
...
intro_text: TextClip = intro_text.with_duration(6).with_start(
    3
)  # Intro for 6 seconds, start after 3 seconds
logo_clip: ImageClip = logo_clip.with_start(intro_text.start + 2).with_end(
    intro_text.end
)  # Logo start 2 second after intro text and stop with it
bird_clip: VideoClip = bird_clip.with_start(
    intro_clip.end
)  # Make bird clip start after intro, duration already known
bird_text: TextClip = bird_text.with_start(bird_clip.start).with_end(
    bird_clip.end
)  # Make text synchro with clip
bunny_clip = bunny_clip.with_start(bird_clip.end)  # Make bunny clip follow bird clip
bunny_text: TextClip = bunny_text.with_start(bunny_clip.start + 2).with_duration(7)
rodents_clip = rodents_clip.with_start(bunny_clip.end)
rodents_text: TextClip = rodents_text.with_start(rodents_clip.start).with_duration(4)
rambo_clip = rambo_clip.with_start(rodents_clip.end - 1.5)
revenge_text: TextClip = revenge_text.with_start(rambo_clip.start + 1.5).with_duration(
    4
)
made_with_text: TextClip = made_with_text.with_start(rambo_clip.end).with_duration(3)
moviepy_clip: ImageClip = moviepy_clip.with_start(made_with_text.start).with_duration(3)
...
#     ]
# )
# quick_compo.preview(fps=24, audio=True)
...
# rambo_clip.preview(fps=10)cleclea
